...
impl Outputs {
    ...
    fn replace(&mut self, idx: usize, expr: Expr) -> Result<(), ExprArgsError> {
        self.0[idx] = expr
            .var_name()
            .ok_or_else(|| ExprArgsError::UnexpectedExprType {
                expected: "var",
                note: Note::NoNote,
            })?
            .clone();
...
impl ExprArgs for CallStmt {
    ...
    fn replace_arg(&mut self, mut idx: usize, expr: Expr) -> Result<(), ExprArgsError> {
        ...
        Err(ExprArgsError::OutOfBounds {
            idx,
            place: "call statement",
            size: self.outputs.len() + self.inputs.len(),
            note: format!(
                "(outputs = {}, inputs={})",
                self.outputs.len(),
                self.inputs.len()
...
impl ExprArgs for CommentLine {
    ...
    fn replace_arg(&mut self, idx: usize, _expr: Expr) -> Result<(), ExprArgsError> {
        Err(ExprArgsError::OutOfBounds {
            idx,
            place: "comment statement",
            size: 0,
            note: "does not have arguments".into(),
        })
    }
}
...
impl StmtConstantFolding for CommentLine {
    fn fold(&self, _: Prime) -> Option<Stmt> {
        None
    }
}
...
impl ExprArgs for AssumeDeterministicStmt {
    ...
    fn replace_arg(&mut self, idx: usize, expr: Expr) -> Result<(), ExprArgsError> {
        ...
        self.0 = expr.var_name().cloned().ok_or_else(|| {
            ExprArgsError::UnexpectedExprType { expected: "var", note: "assume-deterministic statement's argument can only be replaced by variable names".into() }
        })?;
        Ok(())
    }
}
...
impl ExprArgs for PostConditionStmt {
    ...
    fn replace_arg(&mut self, idx: usize, expr: Expr) -> Result<(), ExprArgsError> {
        if idx != 0 {
            return Err(ExprArgsError::OutOfBounds {
                idx,
                place: "post-condition statement",
                size: 1,
                note: Note::NoNote,
            });
        }
