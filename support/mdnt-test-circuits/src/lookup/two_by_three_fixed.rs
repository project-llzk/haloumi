...
impl<F: ff::PrimeField> crate::extraction::ExtractableFixture<F> for Lookup2x3Circuit<F> {
    ...
    where L: haloumi_integration::core::layouter::Layouter<F, Error> + haloumi_integration::core::groups::RegionsGroupHooks<F, midnight_proofs::circuit::Cell, Error = Error> {
        crate::extraction::assign_standard_mul(layouter, &input, config.col_fixed, config.col_a, config.col_b, config.col_c, config.selector)
    }
    fn load_extraction(config: &Self::Config, layouter: &mut impl Layouter<F>) -> Result<(), Error> {
        Lookup2x3Chip::construct(config.clone()).assign_table(layouter)
    }
}
#[cfg(feature = "extraction")]
