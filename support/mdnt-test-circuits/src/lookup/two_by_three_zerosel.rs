...
impl<F: ff::PrimeField> crate::extraction::ExtractableFixture<F> for Lookup2x3ZeroSelCircuit<F> {
    ...
    where L: haloumi_integration::core::layouter::Layouter<F, Error> + haloumi_integration::core::groups::RegionsGroupHooks<F, midnight_proofs::circuit::Cell, Error = Error> {
        crate::extraction::assign_advice_fixed_mul(layouter, &input, config.col_f, config.col_a, config.col_b, config.col_c, None)
    }
    fn load_extraction(config: &Self::Config, layouter: &mut impl Layouter<F>) -> Result<(), Error> {
        Lookup2x3ZeroSelChip::construct(config.clone()).assign_table(layouter)
    }
}
#[cfg(feature = "extraction")]
crate::impl_extractable_fixture!(Lookup2x3ZeroSelCircuit<F>);
